---
title: "The Narrow Garden Cafe"
kraj: "japonia"
miasto: "okagaki"
kategoria: "jedzenie"
tag: ["sweets🍰"]
type: "atrakcja"
lat: 33.84615531759291
lng: 130.62312470194658
image_url: "https://lh3.googleusercontent.com/gps-cs-s/AHRPTWnNMnhbmM8rngNhOePUSLwpMzEX58KLpXy3KzHInVHwCBnmOsOdym9stgs0j1kD8aL-6gqllpnnOBMd4v4GwIQ4O_R_29M5atwMIwDAAS3PL2luix0HdgDFrEIbXdDEguPjZqJIIw=s1360-w1360-h1020-rw"
cena: "Średnio $$"
rezerwacja: "Zalecana"
sezon: "Cały rok"
adres: "3 Chome-7-11 Chuodai, Okagaki, Onga District, Fukuoka 811-4218, Japonia"
link: "https://www.instagram.com/the_narrow_garden/"

rozwijalne_listy:
  - tytul: "Rezerwacja"
    tresc: |
      <ul>
        <li><b>Rezerwacja:</b><p>
        Bezwzględnie wymagana na popołudniową herbatkę (afternoon tea) oraz na ekskluzywny zestaw obiadowy (lunch set).
        </li>

        <li><b>Wskazówka:</b><p>
         Na afternoon tea rezerwacji należy dokonywać z wyprzedzeniem od 14 do 30 dni, ponieważ wolne terminy błyskawicznie znikają.
        </li>

        <li><b>Aplikacje i strony do rezerwacji:</b><p>
         Rezerwacji dokonuje się bezpośrednio poprzez wiadomość prywatną na profilu kawiarni na Instagramie (@the_narrow_garden) lub telefonicznie (080-5209-7751).
        </li>
       </ul>

  - tytul: "Wejście i ceny"
    tresc: |
      
      <ul>
       <li><b>Ceny:</b><p>
        Wstęp wolny po dokonaniu zamówienia z karty lub wcześniejszej rezerwacji zestawu. Ceny napojów i zestawów mieszczą się zazwyczaj w przedziale 1 500 – 4 000 JPY (ok. 40 – 105 PLN) w zależności od wybranego menu (kategoria cenowa: średnio)
       </li>
        
       <li><b>Godziny:</b><p>
         - Dni otwarcia: Niedziela, Poniedziałek, Wtorek, Środa w godzinach 12:00 – 17:00 (ostatnie zamówienie o 16:30). <br>
         - Zamknięte: Czwartek, Piątek, Sobota
       </li>
      </ul>

  - tytul: "Dojazd"
    tresc: |
      <ul>
       <li><b>Z Fukuoki (stacja Hakata) do stacji Ebatsu lub Togawa:</b><br>
         Wsiądź w pociąg linii JR Kagoshima Main Line ze stacji Hakata Station do stacji Ebibotsu Station (czas przejazdu: ok. 35–45 minut pociągiem Rapid/Express). <br>
         - Gdzie kupić bilety na pociąg: Bilety na przejazdy zarezerwujesz na Trip.com lub kupisz na stacji przy użyciu karty IC (Suica, Pasmo, Sugoca, Nimoca).
       </li><p>

       <li><b>Ze stacji Ebibotsu do The Narrow Garden:</b><br>
         - Taksówka: Najwygodniejsza opcja – przejazd taksówką spod stacji Ebibotsu zajmuje około 8–10 minut. <br>
         - Autobus lokalny: Dostępne są lokalne połączenia autobusowe (Town Bus / Community Bus) w kierunku rejonów wiejskich Okagaki.
       </li>
      </ul>

  - tytul: "Przydatne wskazówki"
    tresc: |
     <ul>
      <li><b>Gotówka:</b><br>
        W mniejszych, wiejskich kawiarniach w Japonii karty płatnicze nie zawsze są akceptowane. Warto mieć ze sobą japońskie jeny w gotówce.
      </li><br>

      <li><b>Cisza i relaks:</b><br>
         Kawiarnia jest miejscem stworzonym do wyciszenia – zaleca się zachowanie spokoju, nieużywanie głośnika w telefonie oraz nienaruszanie prywatności innych gości podczas robienia zdjęć. 
      </li><br>

      <li><b>Etykieta fotografowania:</b><br>
         Robienie zdjęć pięknych deserów i widoku na ogród jest mile widziane, jednak unikaj fotografowania twarzy obsługi oraz innych klientów.
      </li>
     </ul>

  - tytul: "Aplikacje"
    tresc: |
      <ul>
        <li><b>Płatności:</b> <p>
         SUICA <a href="https://kkday.tpk.mx/nCHewYjp" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> zamów kartę</a> | karta zbliżeniowa<br>
         PASMO <a href="https://www.pasmo.co.jp/visitors/en/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> zamów kartę </a> | karta zbliżeniowa <br>
        ICOCA <a href="https://www.westjr.co.jp/global/en/howto/icoca/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> zamów kartę</a> | karta zbliżeniowa
        </li>

        <li><b>Nawigacja:</b><p>
          GOOGLE MAPS <a href="https://play.google.com/store/apps/details?id=com.google.android.apps.maps" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a> <br>
          NAVITIME <a href="https://www.navitime.co.jp/pcstorage/html/japan_travel/english/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;">pobierz aplikację</a>
        </li>

        <li><b>Transport:</b><p>
          GO <a href="https://go.goinc.jp/en"_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a> | zamawianie taksówek (polecana) <br> 
          UBER <a href="https://www.uber.com/global/pl/r/japan/cities/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a>  | zamawianie taksówek <br>
          TRIP.COM <a href="https://us.trip.com/?locale=en-us" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a>  | komunikacja miejska <br>
         SHINKANSEN <a href="https://smart-ex.jp/en/lp/app/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a> | kolej dużych prędkości
        </li>

        <li><b>Rezerwacja atrakcji i miejsc:</b><p>
         KLOOK <a href="https://klook.tpk.mx/G9rX9Q6f" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
        KKDAY <a href="https://kkday.tpk.mx/H94SHeIp" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> zobacz stronę</a><br>
          GET YOUR GUIDE <a href="https://www.getyourguide.com/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
         BOOKING.COM <a href="https://www.booking.com/attractions/index.pl.html?label=gog235jc-10CAsodUIbc2FrdXJhLXRyYWRpdGlvbmFsLWphcGFuZXNlSDNYA2i2AYgBAZgBM7gBF8gBDNgBA-gBAfgBAYgCAagCAbgCt5i71AbAAgHSAiQ3OTVhMzgxOC1jOGZlLTRmNWItODVhOC1iNjVhYzg3ZWMwMTPYAgHgAgE&sid=20c8fbb756be0c99ff0818ee28355be8&aid=356980&selected_currency=PLN&source=cross_product_header_link" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
        </li>
      </ul>

galeria_zdjec:
- url: "https://lh3.googleusercontent.com/gps-cs-s/AHRPTWk8crLyII_blCQhKvjC52_tQ6VVUQ6a2LJZu9fP8IgevdaM1YMwJOqiwEYKjFA_145fJpAFcFh2Pi_7jWWbRmxuBxBIYtsN4Ty7_fcgW586_UgMBLveLklPJlBp5y3bP8LyEX_5Wd_prsc=s1360-w1360-h1020-rw"
- url: "https://lh3.googleusercontent.com/gps-cs-s/AHRPTWnU9gD7wmxQzlAiPVd9TlNI2l7oX0OeOEBfSn6QGic8gBLNtvAaPAJh14ykkm_22NmOHRG_2EkXm6EGohAm_A8bBDuNJUN1_cCg60E8t1Ghc1SqLAKXvArQY0V8t_2JrYei6nD62XpDZigJ=s1360-w1360-h1020-rw"
- url: "https://lh3.googleusercontent.com/gps-cs-s/AHRPTWloP0Vq3RRP9REmW3QP26ODsjwD8JuKvNp6VWaVfIgHTxYUi1ywwMIjOCQ-MLAgQpBuhhuoCUF7xN1VNZE_IlAuoBa_pXYFKh0auSPNyIbR9GoNBRbHZyfNrhgF5HYChCVhPYTWF0R35N4m=s1360-w1360-h1020-rw"

- url: "https://lh3.googleusercontent.com/gps-cs-s/AHRPTWkg85qh55asVugNl75xj2c8IHkke_Kp5hEn7vZr5AGHTqdhNeiuiQnYcMNf3fdvqfFbw_mN348NuHVS6zUPoCa8beQBJoCED-7HNDbuUNWGPzVv2n5nhpLSm8GfTfgS37_hZ7I3=s1360-w1360-h1020-rw"

instagram_reel: "https://www.instagram.com/reel/DcJT3D9Ithi/?igsi=MzRlODBiNWFlZA=="

---

### Afternoon tea z francuskimi deserami
Ukryta w cichym zakątku prefektury Fukuoka, The Narrow Garden to wyjątkowa, klimatyczna oaza będąca połaczeniem tradycyjnego japońskiego spokoju z elegancją brytyjskiego afternoon tea oraz francuskimi akcentami deserowymi. Lokal słynie ze zmysłowo oprawionych zestawów podwieczorkowych, eleganckich dań obiadokolacyjnych i autorskiej kawy, serwowanych w otoczeniu kameralnego, starannie wypielęgnowanego ogrodu. To niezwykle kameralne miejsce, stworzone z myślą o osobach szukających estetycznych doznań kulinarnych, gdzie każdy detal — od talerzy dekorowanych świeżymi kwiatami po rzemieślnicze napoje — tworzy iście bajkową atmosfere.

### Fakty i Ciekawostki

* **Kwiatowe niespodzianki na talerzu:** Kawiarnia słynie z pamiątkowych i urodzinowych talerzy dekorowanych świeżymi kompozycjami kwiatowymi (flower surprise plate), idealnych do świętowania wyjątkowych okazji.

* **Architektura wpisana w naturę:** Nazwa "The Narrow Garden" bezpośrednio nawiązuje do unikalnego układu przestrzennego – kawiarnia usytuowana jest wzdłuż długiej, wąskiej działki, gdzie każdy stolik oferuje przemyślany, kadrowany widok na starannie zaprojektowaną zieloną przestrzeń.

* **Sezonowa harmonia smaków:** Menu kawiarni zmienia się wraz z pamiątkowymi porami roku w Japonii. Słynie ona z serwowania dopracowanych autorskich deserów (m.in. domowych tart, rzemieślniczych serników i dań matcha) przygotowywanych z lokalnych składników z rejonu Okagaki.

* **Kawa speciality w wiejskim wydaniu:** Miejsce to przyciąga entuzjastów kawy z całego regionu Kyushu. Ziarna są starannie selekcjonowane i wypalane tak, aby idealnie współgrały z sielską, relaksującą atmosferą ogrodu.

--- 

#### Zobacz poradnik:

<a href="/asia-compass/blog/japan-pay/" style="color: #e98a99; font-weight: bold; text-decoration: underline;">Japonia i płatności</a> &nbsp; | &nbsp; <a href="/asia-compass/blog/japan-transport/" style="color: #e98a99; font-weight: bold; text-decoration: underline;">Japonia jak się poruszać?</a> &nbsp; | &nbsp; <a href="/asia-compass/blog/japan-net/" style="color: #e98a99; font-weight: bold; text-decoration: underline;">Jaki internet w Japonii</a> 