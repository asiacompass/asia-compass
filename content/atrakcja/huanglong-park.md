---
title: "Huanglong National Park"
type: "atrakcja"
kraj: "chiny"
miasto: "huanglong"
lat: 28.7490
lng: 109.9320
kategoria: "natura"
tag: ["unesco🏯", "game🎮",]
cena: "Średnio $$"
image_url: "https://images.pexels.com/photos/29702069/pexels-photo-29702069.jpeg"
rezerwacja: "Tak"
sezon: "Cały rok"
adres: "Furong Ancient Town, Yongshun County, Xiangxi Tujia and Miao Autonomous Prefecture, Hunan, China"
link: "https://pl.trip.com/travel-guide/attraction/yongshun/furong-town-10558754?curr=PLN&locale=pl-PL&poiType=3&scene=ticket&ext-searchpage=1&ext-mixlist=ticket_main&ext-listlocate=0&lasttraceid=1b29adf1-3b22-4833-705c-176df6907066"
map_embed: '<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3355.6402900311855!2d103.82061507525727!3d32.748740973675474!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x36f76bfd0ae5f8a5%3A0x8afc08fba6b1efc!2sHuanglong!5e0!3m2!1spl!2spl!4v1789385346119!5m2!1spl!2spl" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe>'

# Listy rozwijalne pobierane dynamicznie z frontmatter:
rozwijalne_listy:
  - tytul: "Rezerwacja"
    tresc: |
      <ul>
        <li><strong>Rezerwacja:</strong><br>
         Bezwzględnie wymagana z wyprzedzeniem. W parku obowiązuje rygorystyczny dzienny limit odwiedzających. Na miejscu przy kasach bilety nie są sprzedawane dla turystów indywidualnych. <br>
         Rezerwuj z 7–14 dniowym wyprzedzeniem, szczególnie w szczycie sezonu (wrzesień – październik, gdy liście zmieniają kolory) oraz w trakcie chińskich świąt narodowych.
        </li><br>

        <li><b>Gdzie rezerwować:</b><br>
         Bilety wstępu oraz wjazd kolejką linową najwygodniej, bez kolejki i z natychmiastowym potwierdzeniem zarezerwujesz w aplikacji Trip.com <a href="https://www.trip.com/t/XsDFf0VQMW2" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold;">[zarezerwuj bilet]</a> lub Klook <a href="https://affiliate.klook.com/sl/TIcym2P" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold;">[zarezerwuj bilet]</a>
        </li>
      </ul>

  - tytul: "Wejście i ceny"
    tresc: |
      <ul>
        <li><b>Ceny biletów:</b><br>
         - Sezon wysoki (1 kwietnia – 30 listopada): Wstęp ok. 170 CNY (ok. 94 PLN); Kolejka linowa w górę: 80 CNY (ok. 44 PLN), w dół: 40 CNY (ok. 22 PLN) <br>
         - Sezon niski (1 grudnia – 31 marca): Wstęp ok. 60 CNY (ok. 33 PLN)
        </li><br>

        <li><b>Godziny otwarcia:</b><br>
         Codziennie 08:00 – 17:00 (w sezonie letnim wejście bywa otwierane od 07:30). <br>
         Najlepsza pora na wizytę: Od września do października (spektakularne jesienne kolory lasu i wysoki poziom wody w basenach). Najlepiej rozpocząć zwiedzanie rano o 08:00.
        </li>

        <li><b>Gdzie kupić:</b><br>
         Najtaniej i bez kolejki oficjalne bilety oraz przejazdy pociągiem kupisz przez Trip.com – Zarezerwuj wejście z wyprzedzeniem.
        </li>
      </ul>

  - tytul: "Dojazd"
    tresc: |
      <ul>
        <li><b>Z Chengdu do Huanglong:</b><br>
         - Najszybciej dojedziesz superszybkim pociągiem (CRH) ze stacji Chengdu East bezpośrednio do stacji Huanglongjiuzhai Railway Station (czas przejazdu to zaledwie ok. 1,5 do 2 godzin!). 
         <br> 
         - Ze stacji Huanglongjiuzhai do bram parku narodowego Huanglong dojedziesz w około 30–40 minut bezpośrednim autobusem turystycznym lub taksówką (Didi/Alipay).
        </li><br>

        <li><b>Aplikacje do przejazdów i biletów:</b><br>
         Do nawigacji używaj Amap (Gaode Maps). Transakcje i bilety na miejscu opłacisz zbliżeniowo przez aplikację Alipay lub WeChat Pay.
        </li><br>

  - tytul: "Przydatne informacje"
    tresc: |
      <ul>
        <li><b>Choroba wysokościowa:</b><br>
         łówna atrakcja (Basen Pięciu Kolorów) znajduje się na wysokości około 3500 m n.p.m. Pamiętaj o wolnym tempie chodzenia, odpowiednim nawodnieniu i zabraniu ze sobą małej puszki z tlenem (można je łatwo kupić w sklepikach przed wejściem).
        </li><br>

        <li><b>Rekomendowana trasa:</b><br>
         Wjedź w górę kolejką linową (Cable Car), następnie przejdź płaskim drewnianym pomostem (ok. 2,5 km) do Basenu Pięciu Kolorów, a potem schodź pieszo w dół wzdłuż całej doliny, podziwiając po drodze wszystkie kaskady i baseny.
        </li><br>

        <li><b>Zakaz schodzenia ze ścieżek:</b><br>
         Formacje trawertynowe są niezwykle delikatne. Obowiązuje bezwzględny zakaz dotykania wody, schodzenia z drewnianych pomostów oraz śmiecenia.
        </li>

        <li><b>Ciepła odzież:</b><br>
         Pogoda w górach zmienia się błyskawicznie. Nawet w lecie warto zabrać ze sobą kurtkę przeciwwiatrową i przeciwdeszczową.
        </li>

  - tytul: "Aplikacje"
    tresc: |
      <ul>
        <li><b>Płatności:</b> <p>
         WeChat Pay <a href="https://www.wechat.com/en" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
         Alipay <a href="https://render.alipay.com/p/yuyan/180020040001212700/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikacje </a>
        </li>

        <li><b>Nawigacja:</b><p>
          Amap (Gaode) <a href="https://play.google.com/store/apps/details?id=com.autonavi.minimap&hl=pl" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a> | piesza nawigacja<br>
          Baidu Maps <a href="https://play.google.com/store/apps/details?id=com.baidu.BaiduMap&hl=pl" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;">pobierz aplikację</a>
          
        </li>

        <li><b>Transport:</b><p>
          Trip.com <a href="https://www.trip.com/trains/?locale=en-US&curr=PLN" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a> | komunikacja miejska <br> 
          Alipay Didi <a href="https://web.didiglobal.com/" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a>  | zamawianie taksówek <br>
        </li>

        <li><b>Rezerwacja atrakcji i miejsc:</b><p>
          Trip.com <a href="https://www.trip.com/?locale=en-US&curr=PLN" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
          Klook <a href="https://klook.tpk.mx/G9rX9Q6f" target="_blank" rel="noopener" style="color: #e98a99; font-weight: bold; text-decoration: underline;"> pobierz aplikację</a><br>
        </li>
      </ul>
 
galeria_zdjec:
  - url: "https://images.pexels.com/photos/34222098/pexels-photo-34222098.jpeg"
  - url: "https://images.pexels.com/photos/29767791/pexels-photo-29767791.jpeg"
  - url: "https://images.pexels.com/photos/29767796/pexels-photo-29767796.jpeg"
  - url: "https://images.pexels.com/photos/31654300/pexels-photo-31654300.jpeg"
  - url: "https://images.pexels.com/photos/29767800/pexels-photo-29767800.jpeg"

instagram_reel: "https://www.instagram.com/reel/Dak43a-o-Jc/?igsi=MzRlODBiNWFlZA=="

---

### Dolina Żółtego Smoka
Ukryta w wysokich partiach gór Min Shan w prowincji Syczuan, Park Narodowy Huanglong to wpisana na listę światowego dziedzictwa UNESCO kraina, która wygląda jak wyjęta z sennych marzeń. Słynie przede wszystkim z gigantycznych, uskokowych tarasów trawertynowych, w których krystalicznie czysta, wapienna woda tworzy setki naturalnych basenów. W zależności od pory dnia, kąta padania światła i pory roku, woda mieni się niesamowitą paletą barw – od intensywnego turkusu i szmaragdu, po ciepły błękit i złoto. W tle rozciągają się majestatyczne, ośnieżone szczyty górskie oraz gęste, pierwotne lasy, w których wciąż żyją dzikie pandy wielkie i rokselany złociste.  
Krajobrazy Huanglong (oraz sąsiedniej doliny Jiuzhaigou) były bezpośrednią inspiracją dla twórców gry **Genshin Impact** przy projektowaniu regionu Liyue (szczególnie Luhua Pool).

### Fakty i Ciekawostki

* **Legenda o Złotym Smoku:** Nazwa Huanglong dosłownie oznacza „Żółty Smok”. Według lokalnych podań taoistycznych żółty smok pomógł opanować wielką powódź, a po wykonaniu zadania spoczął w tym miejscu. Żółtawo-złote formacje trawertynowe ciągnące się przez 3,6 km doliny wyglądają z lotu ptaka dokładnie jak łuski ogromnego, śpiącego smoka.  
* **Największe baseny trawertynowe w Chinach:** Kompleks liczy ponad 3400 wapiennych basenów. Najbardziej spektakularny z nich, Wucai Chi (Basen Pięciu Kolorów), leży na wysokości niemal 3500 m n.p.m. i mieści aż 693 pojedyncze zbiorniki wodne.   
* **Geologiczna unikalność:** rawertyn w Huanglong powstawał przez tysiące lat w wyniku osadzania się węglanu wapnia z wody źródlanej spływającej z lodowca góry Xuebaoding (5588 m n.p.m.). To jeden z najbardziej unikalnych i najlepiej zachowanych krajobrazów krasowych na świecie.   
* **Świątynia na wysokościach:** U stóp najwyższych basenów stoi zabytkowa, taoistyczna Świątynia Huanglong (Huanglong Ancient Temple), wzniesiona za czasów dynastii Ming, która nadaje całemu krajobrazowi niezwykle mistycznego charakteru.  


#### Zobacz poradnik:

<a href="/asia-compass/blog/china-pay/" style="color: #e98a99; font-weight: bold; ">Jak płacić w Chinach?</a> &nbsp; | &nbsp; <a href="/asia-compass/blog/china-transport/" style="color: #e98a99; font-weight: bold; ">Chiny i transport</a> &nbsp; | &nbsp; <a href="/asia-compass/blog/china-net/" style="color: #e98a99; font-weight: bold; ">Chiny i internet</a> &nbsp; | &nbsp; <a href="/asia-compass/blog/china-app/" style="color: #e98a99; font-weight: bold; ">Aplikacje przydatne w Chinach</a>


<!--more-->